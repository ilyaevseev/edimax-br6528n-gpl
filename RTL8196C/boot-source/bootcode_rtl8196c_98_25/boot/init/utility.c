...
//	else if  (!memcmp(pHeader->signature, FW_SIGNATURE_WITH_ROOT, SIG_LEN))
	else if  (!memcmp(pHeader->signature, FW_SIGNATURE_WITH_ROOT, 2))//EDX BSP check "cr" only
		ret=2;
	else 
...
//	else if  (!memcmp(pHeader->signature, image_sig_root, SIG_LEN))
	else if  (!memcmp(pHeader->signature, image_sig_root, 2)) //EDX BSP check "cr" only
		ret=2;
	else{
